    ...
    fn execute_mouse_action(
        ...
        match action {
            ...
            MouseAction::MiddleClickPaste { pane_id: _ } => {
                if let Some(paste_cmd) = &tab.paste_command {
                    let mut cmd_parts = paste_cmd.split_whitespace();
                    let program = cmd_parts.next().unwrap_or("");
                    let args: Vec<&str> = cmd_parts.collect();
                    let clipboard_text = std::process::Command::new(program)
                        .args(&args)
                        .output()
                        .ok()
